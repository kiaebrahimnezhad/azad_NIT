...
function LoginForm() {
  ...
  const submitHandler = () => {
    iamApi
      .post<LoginResponse>("/login", userInf)
      .then((response) => {
        const { token, userType } = response.data;

        login({ username: userInf.username, token, userType });
      ...
      .catch((err) => {
        setSubmitResult({
          message: userSafeErrorMessage(err, "نام کاربری یا رمز عبور اشتباه است."),
          show: true,
          success: false,
        });
      });
...
function ForgotForm({ onToast }: { onToast?: (message: string, type?: ToastType) => void }) {
  ...
  const emailSubmitHandler = () => {
    ...
    iamApi
      .post<LoginResponse>("/login/forgot-password", req)
      .then(() => {
      ...
      .catch((err) => {
        if (isAxiosErrorWithMessage(err) && err.response?.data?.message === "User not found.") {
          setEmailRes({
            show: true,
            success: false,
  ...
  const otpSubmitHandler = () => {
    iamApi
      .post<otpResponse>("/login/forgot-password-otp", userInf)
      .then((resp) => {
