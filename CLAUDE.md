...
**Certificate generation requires a real LibreOffice install on the machine running `core`, not just the npm package.** When a student passes an exam, `submitExamController` (`backend/core/src/controllers/examController.ts`) fills `backend/core/uploads/certification.docx` via `docxtemplater`, then converts it to PDF via `libreoffice-convert` (`convertAsync`). That package only shells out to a locally installed LibreOffice (`soffice`) binary — it does not bundle a PDF converter itself. If LibreOffice isn't installed (or isn't on `PATH`), the conversion throws, the exam submission still succeeds/scores normally, but the response falls back to "You passed, but there was an issue generating the certificate." with no certificate ever created. Install LibreOffice (free, from the official site) on any machine expected to run `core` and issue certificates, including deployment targets — this isn't optional/dev-only. Certificates are per-exam, not per-course: the `Certificate` model is keyed `@@unique([username, eid])`, so a student gets one certificate per passed exam, not one after passing every exam in a course.
